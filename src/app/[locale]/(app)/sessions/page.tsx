...
import { apiListSessions, getStaff, hasPermission } from "@/lib/api";
...
export default function SessionsPage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setLoading(true);
      try {
        const data = await apiListSessions(q, status, offset);
        if (!cancelled) {
          setItems(data.items || []);
            ...
            {loading ? (
              ...
              items.map((r) => (
                <tr key={String(r.id)} className="border-t border-[var(--border)]">
                  ...
                  <td className="px-3 py-2">{String(r.authMethod || "—")}</td>
